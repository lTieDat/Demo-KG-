...
def check_checkpoint_compatibility(checkpoint_path, target_dataset="code-ptit-100k"):
    ...
    try:
        ...
        if info['dataset'] == target_dataset:
            ...
            info['compatibility_issues'].append(f"Dataset mismatch: {info['dataset']} != {target_dataset}")
        ...
        if info['embedding_size'] != 'Unknown' and info['embedding_size'] != 32:
            info['compatibility_issues'].append(f"Embedding size mismatch: {info['embedding_size']} != 32")
    ...
    try:
        ...
        if 'config' in checkpoint:
            ...
            if hasattr(original_config, 'final_config_dict'):
                ...
                config_dict = dict(original_config) if hasattr(original_config, '__iter__') else {}
        ...
        try:
            ...
            st.success(f"✅ Đã load model thành công! Epoch: {checkpoint.get('epoch', 'N/A')}")
            ...
            if os.path.exists(config_file):
                ...
                try:
                    ...
                    st.warning("⚠️ Loaded với strict=False (có thể một số weights không match)")
                ...
                st.error("❌ Không tìm thấy config.yaml và không thể load từ checkpoint")
                return None, None, None
...
if load_method == "Từ checkpoint (mới)":
    ...
    compatible_models, incompatible_models = get_compatible_checkpoints(RECBOLE_MODEL_DIR)
    ...
    if compatible_models:
        ...
        if selected_model:
            ...
            with st.sidebar.expander("📋 Thông tin Checkpoint"):
                ...
                st.write(f"**Users:** {info['user_count']:,}" if info['user_count'] != 'Unknown' else "**Users:** Unknown")
                st.write(f"**Entities:** {info['entity_count']:,}" if info['entity_count'] != 'Unknown' else "**Entities:** Unknown")
                st.write(f"**Relations:** {info['relation_count']:,}" if info['relation_count'] != 'Unknown' else "**Relations:** Unknown")
        
        # Hiển thị thông tin về model không tương thích nếu có
        if incompatible_models:
            with st.sidebar.expander(f"⚠️ {len(incompatible_models)} model không tương thích"):
...
if (
    ...
    if input_method == "Dropdown mã sinh viên":
        ...
        if student_code_input:
            ...
            if user_id:
                st.success(f"✅ Tìm thấy sinh viên: **{student_code_input}** (User ID: {user_id})")
            else:
                st.error(f"❌ Không tìm thấy mã sinh viên: **{student_code_input}**")
                st.info("💡 Gợi ý: Hãy thử nhập chính xác mã sinh viên từ danh sách hoặc sử dụng chế độ Dropdown")
